...
actor FATAllocationTable: FreeSpaceMapping {
    ...
    enum FATError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidBPB: "Invalid FAT BPB (Boot Parameter Block)."
            case .notFAT32: "Volume is not FAT32."
            case .readError: "Failed to read FAT data from disk."
            }
        }
    }
    ...
    private func initializeIfNeeded() throws {
        ...
        do {
            try parseBPB()
            isInitialized = true
            let parsedClusterCount = clusterCount
            logger.info("Successfully parsed FAT32 BPB. Cluster count: \(parsedClusterCount)")
        } catch {
    ...
    private func parseBPB() throws {
        ...
        let bytesRead = bpbData.withUnsafeMutableBytes { buffer in
    ...
    private var dataRegionOffset: UInt64 {
        bpbOffset + UInt64(reservedSectors) * UInt64(bytesPerSector) + UInt64(numberOfFATs) * UInt64(fatSize32) *
    ...
    private func _populateStream(continuation: AsyncThrowingStream<FreeSpaceRange, Error>.Continuation) async {
        do {
            ...
            var currentFreeStartCluster: UInt32?
            ...
            while clusterIndex < clusterCount + 2 {
                ...
                for i in 0 ..< Int(entriesToRead) {
                    ...
                    let entryValue = readLittleEndianUInt32(bytes, at: entryOffset) & 0x0FFF_FFFF

                    if entryValue == 0x0000_0000 {
                        // Free cluster
