...
actor APFSSpaceManager: FreeSpaceMapping {
    ...
    enum APFSError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidSuperblock: return "Invalid APFS Container Superblock."
            case .unsupportedSpaceman: return "Unsupported APFS Spaceman format or structure."
            case .readError: return "Failed to read APFS data from disk."
            }
        }
    }
    ...
    private func initializeIfNeeded() throws {
        ...
        do {
            ...
            logger.warning("APFS Spaceman parsing unavailable: \(error.localizedDescription). Falling back to sequential linear scanning.")
            throw error
        }
    }
