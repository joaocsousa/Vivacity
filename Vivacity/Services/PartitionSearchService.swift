    ...
    func findPartitions(on devicePath: String, reader: PrivilegedDiskReading) async throws -> [StorageDevice] {
        ...
        if headerRead == blockSize {
            ...
            if String(bytes: signatureBytes, encoding: .ascii) == "EFI PART" {
                ...
                let numEntries = gptHeaderBlock.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: 80, as: UInt32.self) }
                ...
                let arrayStartLBA = gptHeaderBlock.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: 72, as: UInt64.self) }
                ...
                if arrayRead == arrayBytesToRead {
                    for i in 0..<Int(numEntries) {
                        ...
                        let firstLBA = arrayBlock.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: entryStart + 32, as: UInt64.self) }
                        // Last LBA at offset 40 (8 bytes)
                        let lastLBA = arrayBlock.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: entryStart + 40, as: UInt64.self) }
                        
                        let partitionOffset = firstLBA * UInt64(blockSize)
                        let partitionSize = (lastLBA - firstLBA + 1) * UInt64(blockSize)
                        
                        let formattedOffset = ByteCountFormatter.string(fromByteCount: Int64(partitionOffset), countStyle: .file)
